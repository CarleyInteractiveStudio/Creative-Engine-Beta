...
(function() {
    CreativeEngine.API.registrarVentana({
        ...
        alAbrir: function(panel) {
            panel.texto("Welcome to my tool!", { negrita: true, color: "#3498db" });

            panel.fila((f) => {
                f.boton("Say Hello", () => alert("Hello!"));
...
## 🎮 2. Runtime Libraries (New Functions for Scripts)
...
```ces
engine motor;
go "MyCalculator"; // We import the library

start() {
    let result = add(10, 5); // Lib functions are global now
    log("Result: " + result);
    log("My ID is: " + generateID());
}
```
