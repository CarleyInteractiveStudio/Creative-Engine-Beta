...
test('CMModelConverter & AssetImportModalWindow - Complete GLB Import Suite', async () => {
    ...
    assert.strictEqual(result.textures[0].name, 'CharacterTexture.png', 'Texture name should match');
    assert.strictEqual(result.cmData.materials[0].texturePath, 'CharacterTexture.png', 'Material texture link should be set');
