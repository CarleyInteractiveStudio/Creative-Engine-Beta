...
export const Gizmos = {
    ...
    drawWireSphere(ctx, center, radius, rotation = {x:0, y:0, z:0}, color = 'rgba(0, 255, 255, 0.8)') {
        ...
        const drawRing = (axis) => {
            ...
            for (let i = 0; i <= segments; i++) {
                ...
                if (lastWorld) drawLineClipped(ctx, lastWorld, currentWorld, color, 1);
    ...
    drawWireTriangle(ctx, center, size, rotation = {x:0, y:0, z:0}, color = 'rgba(0, 255, 255, 0.8)') {
        ...
        drawLineClipped(ctx, points[0], points[1], color, 1);
        drawLineClipped(ctx, points[1], points[2], color, 1);
        drawLineClipped(ctx, points[2], points[0], color, 1);
    },
    ...
    drawWirePlane(ctx, center, size, rotation = {x:0, y:0, z:0}, color = 'rgba(255, 255, 255, 0.5)') {
        ...
        for (let i = 0; i < 4; i++) drawLineClipped(ctx, points[i], points[(i + 1) % 4], color, 1);
    },
    ...
    drawWireCapsule(ctx, center, radius, height, rotation = {x:0, y:0, z:0}, color = 'rgba(0, 255, 255, 0.8)') {
        ...
        this.drawWireSphere(ctx, { x: center.x + tPos[0], y: center.y + tPos[1], z: (center.z||0) + tPos[2] }, radius, rotation, color);
        this.drawWireSphere(ctx, { x: center.x + bPos[0], y: center.y + bPos[1], z: (center.z||0) + bPos[2] }, radius, rotation, color);
    }
};
