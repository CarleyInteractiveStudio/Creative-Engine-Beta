            ...
            for (const key in ley) {
                ...
                if (key !== 'materia' && key !== 'scriptInstance' && typeof ley[key] !== 'function') {
                    if (ley.constructor.name === 'Tilemap' && key === 'layers') {
                        ...
                        } else if (key === 'visibilityMask') {
                            leyData.properties[key] = ley[key] ? Array.from(ley[key]) : null;
                        }
        ...
        if (leyData.type === 'CustomComponent') {
            ...
            if (ComponentClass) {
                ...
                if (leyData.type === 'Tilemap') {
                    ...
                    if (leyData.properties.visibilityMask) {
                        newLey.visibilityMask = new Float32Array(leyData.properties.visibilityMask);
                    }
