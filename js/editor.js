...
import { initialize as initializeAssetBrowser, updateAssetBrowser, getCurrentDirectoryHandle, handleContextMenuAction as handleAssetContextMenuAction, revealAsset } from './editor/ui/AssetBrowserWindow.js';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    updateScene = function(rendererInstance, isGameView) {
        ...
        if (isGameView) {
            ...
            if (cameras.length > 0) {
                // For simplicity, we'll use the first active camera found.
                // A more robust solution would handle multiple cameras based on depth.
                activeCamera = cameras[0];
            }
        }

        rendererInstance.drawScene(SceneManager.currentScene, activeCamera, isGameView);

        // The overlay is editor-specific, so it stays here.
        if (!isGameView) {
    ...
    async function initializeEditor() {
        ...
        const ids = [
            ...
            'settings-agent-type-list', 'new-agent-type-name', 'add-agent-type-btn',
             'prefs-theme', 'prefs-custom-theme-picker', 'prefs-color-bg', 'prefs-color-header',
