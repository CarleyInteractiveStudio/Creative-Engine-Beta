...
const typeMap = {
    ...
    'fuenteDeAudio': 'AudioSource',
    'fonteDeÁudio': 'AudioSource',
    'источникЗвука': 'AudioSource',
    '音频源': 'AudioSource',
    'ParticleSystem': 'ParticleSystem',
    'sistemaDeParticulas': 'ParticleSystem',
    'sistemaDePartículas': 'ParticleSystem',
    'системаЧастиц': 'ParticleSystem',
    '粒子系统': 'ParticleSystem',
    'Attack': 'Attack',
    'ataque': 'Attack',
    'атака': 'Attack',
    ...
    'barraDeProgreso': 'ProgressBar',
    'barraDeProgresso': 'ProgressBar',
    'полосаПрогресса': 'ProgressBar',
    '进度条': 'ProgressBar'
};
...
function transpileBlock(block, componentShortcuts, publicVars, privateVars, importedLibs, RuntimeAPIManager, customFunctions = []) {
    ...
    body = body.replace(/(?<![.\w])(esperar|wait|aguardar|ждать|等待)\s*\(/g, 'await this.esperar(');
...
export function transpile(code, scriptName = 'unnamed.ces') {
    ...
    const varRegex = /^\s*(?:(public|private|publico|público|bublico|bublica|piblico|piblica|privado|закрытый|закрытая|открытый|открытая|公开|私有)\s+)?(?!(?:si|sino|se|senão|mientras|enquanto|para|cada|go|ve|engine|motor|двигатель|引擎|если|иначе|пока|для|如果|否则|当|对于|crear|create|criar|создать|创建|esperar|aguardar|ждать|等待)(?![.\w\u00C0-\u017F\u0400-\u04FF\u4E00-\u9FA5]))([a-zA-Z_\u00C0-\u017Fа-яА-Я一-龥][\w\u00C0-\u017Fа-яА-Я一-龥]*)\s+([a-zA-Z_\u00C0-\u017Fа-яА-Я一-龥][\w\u00C0-\u017Fа-яА-Я一-龥]*)\s*(?:=\s*([^;\n\r]+))?;?/gm;
    let varMatch;
