...
/**
 * Muestra la ventana flotante de guia para novatos.
...
export function show(dom) {
    ...
    panel.texto(L.get('MSG_NOVATO', 'Hola, has activado la casilla de novatos. Aqui tienes una guia basica para empezar.'), { negrita: true });

    panel.separador();

    panel.columna({ gap: '15px' }, (col) => {
        col.texto(L.get('CONSEJO_NOVATO_1', '1. Crea una Materia en la Jerarquia para empezar tu objeto.'));
        col.texto(L.get('CONSEJO_NOVATO_2', '2. Anade una Ley (Componente) en el Inspector para darle propiedades.'));
        col.texto(L.get('CONSEJO_NOVATO_3', '3. Usa Scripts (.ces) en el Navegador para programar la logica.'));
    });
    ...
    panel.fila({ gap: '10px' }, (f) => {
        ...
        f.boton(L.get('IR_A_DOCUMENTACION', 'Ver Documentacion'), () => {
            // Activar boton de ayuda del editor
            if (dom.menuDocs) dom.menuDocs.click();
        }, { color: '#2ecc71', ancho: '50%' });
    });

    panel.separador();
    panel.texto(L.get('ACCESOS_RAPIDOS', 'Accesos Rapidos:'), { tamano: '0.9em' });
    ...
    panel.checkbox(L.get('DESACTIVAR_GUIA', 'Desactivar guia para este proyecto'), isDeactivated, (checked) => {
