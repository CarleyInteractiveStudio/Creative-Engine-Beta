...
export async function showBuildDialog(projectConfig, onConfirm) {
    ...
    const content = `
        <div class="advanced-build-dialog" style="min-width: 450px; display: flex; flex-direction: column; height: 100%; box-sizing: border-box;">
            ...
            <div style="flex-grow: 1; overflow-y: auto; padding-right: 5px; margin-bottom: 15px;">
            ...
            <div class="dialog-footer" style="margin-top: auto; display: flex; justify-content: flex-end; gap: 10px;">
                <!-- Filled dynamically by updateBuildDialogButtons -->
            </div>
        </div>
    `;

    // Create the build panel as a genuine floating-panel
    const panelId = 'build-config-panel';
    // Remove if already exists to prevent duplicate windows
    ...
    const panelElement = createFloatingPanel(panelId, {
        title: L.get('BUILD_CONFIG', 'Configuración Avanzada de Build'),
        content: content,
        width: 500,
        height: 480,
        top: 100,
    ...
    const footer = panelElement.querySelector('.dialog-footer');

    const updateBuildDialogButtons = () => {
        const activeTabBtn = panelElement.querySelector('.build-tab-btn.active');
        ...
        btnCancel.onclick = () => panelElement.remove();
        ...
        if (activeTabId === 'tab-general') {
            ...
            btnNext.onclick = () => {
                const nextTab = panelElement.querySelector('[data-tab="tab-scenes"]');
            ...
            btnBack.onclick = () => {
                const prevTab = panelElement.querySelector('[data-tab="tab-general"]');
            ...
            btnNext.textContent = L.get('SIGUIENTE', 'Siguiente');
            btnNext.onclick = () => {
                const nextTab = panelElement.querySelector('[data-tab="tab-splash"]');
                if (nextTab) nextTab.click();
            };
            ...
            btnBack.textContent = L.get('ATRÁS', 'Atrás');
            btnBack.onclick = () => {
                const prevTab = panelElement.querySelector('[data-tab="tab-scenes"]');
                if (prevTab) prevTab.click();
            };
            const btnNext = document.createElement('button');
            btnNext.className = 'dialog-button primary';
            btnNext.textContent = L.get('SIGUIENTE', 'Siguiente');
            btnNext.onclick = () => {
                const nextTab = panelElement.querySelector('[data-tab="tab-export"]');
                if (nextTab) nextTab.click();
            };
            footer.appendChild(btnBack);
            footer.appendChild(btnNext);
            ...
            btnBack.textContent = L.get('ATRÁS', 'Atrás');
            btnBack.onclick = () => {
                const prevTab = panelElement.querySelector('[data-tab="tab-splash"]');
                if (prevTab) prevTab.click();
            };
            ...
            btnBuild.onclick = async () => {
                const options = {
                    appName: panelElement.querySelector('#build-app-name').value,
                    appAuthor: panelElement.querySelector('#build-app-author').value,
                    appVersion: panelElement.querySelector('#build-app-version').value,
                    appIcon: panelElement.querySelector('#build-app-icon').value,
                    method: panelElement.querySelector('#build-method').value,
                    includeUnusedAssets: panelElement.querySelector('#include-unused').checked,
                    runAfterBuild: panelElement.querySelector('#run-after').checked,
                    resourceLoadingMode: panelElement.querySelector('#resource-loading-mode').value,
                    exportTarget: panelElement.querySelector('input[name="export-target"]:checked').value,
                    startScene: panelElement.querySelector('input[name="start-scene"]:checked')?.value,
                    includedScenes: Array.from(panelElement.querySelectorAll('.include-scene:checked')).map(el => el.dataset.path),
                    splashScreens: {
                        show: panelElement.querySelector('#show-splash').checked,
                        showEngineLogo: panelElement.querySelector('#show-engine-logo').checked,
                        engineLogoDuration: parseFloat(panelElement.querySelector('#engine-logo-duration').value) || 3,
                        list: Array.from(panelElement.querySelectorAll('.splash-item')).map(item => ({
    ...
    // Handle Tabs
    const tabs = panelElement.querySelectorAll('.build-tab-btn');
    const contents = panelElement.querySelectorAll('.build-tab-content');
    tabs.forEach(tab => {
        tab.onclick = () => {
            ...
            panelElement.querySelector(`#${tab.dataset.tab}`).style.display = 'block';
            updateBuildDialogButtons();
        };
    });

    // Handle Splash Toggle
    const showSplash = panelElement.querySelector('#show-splash');
    const splashSettings = panelElement.querySelector('#splash-settings');
    showSplash.onchange = () => splashSettings.style.display = showSplash.checked ? 'block' : 'none';

    // Handle Icon Selection
    panelElement.querySelector('#btn-select-icon').onclick = () => {
        if (window.openAssetSelector) {
            window.openAssetSelector((handle, path) => {
                panelElement.querySelector('#build-app-icon').value = path;
            }, { filter: ['.png', '.jpg', '.jpeg', '.ceSprite'], title: L.get('SELECCIONAR_ICONO', 'Seleccionar Icono') });
        }
    };

    // Handle Add Splash
    panelElement.querySelector('#btn-add-splash').onclick = () => {
        if (window.openAssetSelector) {
            window.openAssetSelector((handle, path) => {
                const list = panelElement.querySelector('#custom-splash-list');
    ...
    panelElement.querySelectorAll('.btn-remove-splash').forEach(btn => {
